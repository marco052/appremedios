...
class _AddMedicineState extends State<AddMedicine>
  ...
  List<Schedule> scheduleList = [
    new Schedule('12:00', DateTime.now().weekday)
  ];
            ...
            onPressed: () {
              setState(() {
                this.scheduleList.add(
                  new Schedule('', DateTime.now().weekday)
                );
            ...
            children: [
              Container(
                ...
                child: Column(
                  children: [
                    SizedBox( height: 20 ),
                    Text('Adicionar Medicamento', style: AppTextStyles.interRegularTitle),
                    SizedBox( height: 20 ),
                    Container(
                      width: MediaQuery.of(context).size.width - 20,
                      height: 70,
                      ...
                      child: Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            Text(this._selectedMedicine.name,  style: AppTextStyles.interText,),
                            TextButton.icon(
                              icon: Icon(Icons.find_in_page, color: AppColors.primary),
                              onPressed: () => {
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                  ...
                  child: Column(
                    children: [
                      Container(
                        ...
                        child: Text('Tipo', style: AppTextStyles.interBoldLabel,)
                      ),
                      DropdownButtonFormField<String>(
                        value: _typeValue,
                        items: _types.map((String dropType) =>
                        ...
                        onChanged: (newItemsname) {
                          if (newItemsname != null)
                            setState(() {
                              ...
                              if (newItemsname == 'Único' && this.scheduleList.length > 1) {
                                this.scheduleList.removeRange(1, this.scheduleList.length);
                              }
                            });
                        },
                      ),
                      ...
                      Row(
                        children: [
                          Column(
                            children: [
                              Container(
                                ...
                                alignment: Alignment.centerLeft,
                                child: Text('Dosagem', style: AppTextStyles.interBoldLabel,)
                              ),
                              Container(
                                width: (MediaQuery.of(context).size.width - 20) * 0.6,
                                height: 50,
                                child: DropdownButtonFormField<String>(
                                  ...
                                  items: _dosageType.map((String dropDosagem) =>
                                    DropdownMenuItem<String>(
                                      value: dropDosagem,
                          ...
                          SizedBox(
                            width: (MediaQuery.of(context).size.width - 20) * 0.05,
                          ),
                          Column(
                            children: [
                              Container(
                                ...
                                alignment: Alignment.centerLeft,
                                child: Text('Quantidade', style: AppTextStyles.interBoldLabel,)
                              ),
                              Container(
                                width: (MediaQuery.of(context).size.width - 20) * 0.35,
                                height: 50,
                      ...
                      Container(
                        alignment: Alignment.centerLeft,
                        child: Text('Agendamento', style: AppTextStyles.interBoldLabel,)
                      ),
                      Column(
                        children: scheduleList.mapIndexed((index, sch) =>
                          Column(
                            children: [
                              Row(
                                children: [
                                  Container(
                                    ...
                                    child: DropdownButtonFormField<int>(
                                      value: this.scheduleList[index].weekDay,
                                      items: week.values.mapIndexed((key, value) => 
                                        DropdownMenuItem<int>(
                                          value: this.week.keys.toList()[key],
                                          child: Text(value),
                                        )
                                      ).toList(),
                                      onChanged: (newValue) {
                                        if (newValue != null)
                                          this.scheduleList[index].weekDay = newValue;
                                      },
                                    ),
                                  ),
                                  SizedBox(
                                  ...
                                  Container(
                                    ...
                                    child: TextField(
                                      ...
                                      onChanged: (value) => {
                                        this.scheduleList[index].time = value
                                      },
                                      decoration: InputDecoration(
                                        hintText: '12:00',
                                        helperStyle: AppTextStyles.placeholder
                                      ),
                                    ),
                                  ),
                                  SizedBox(
